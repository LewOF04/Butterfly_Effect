...
[Preserve]
public class MurderNPC : NPCAction
{
    public MurderNPC() : base('N'){}
    ...
    protected override List<int> utilityNegTraits => new List<int>{1,5};
    protected override List<int> successPosTraits => new List<int>{5}; 
    ...
    protected override float computeUtility(IAgent performer, IAgent target)
    {
        ...
        effectors.Add(ActionMaths.calcMultiplier(target.stats.condition, 0f, 100f, 2f, 0.25f)); weights.Add(0.8f);
        ...
        effectors.Add(ActionMaths.calcMultiplier(performer.stats.happiness, 0f, 100f, 2f, 0.1f)); weights.Add(0.6f);
        effectors.Add(ActionMaths.calcMultiplier(performer.attributes.morality, 0f, 100f, 2f, 0.1f)); weights.Add(0.7f);

        effectors.Add(ActionMaths.calcMultiplier(performer.attributes.constitution, 0f, 100f, 0.25f, 2f)); weights.Add(0.4f);
        effectors.Add(ActionMaths.calcExpMultiplier(performer.attributes.morality, 100f, 0f, 3f, 0.1f, 5f)); weights.Add(0.9f);
    ...
    protected override void innerPerformAction(float percentComplete)
    {
        ...
        string description = performer.fullName + " spent " + (percentMulti*timeToComplete).ToString("0.00") + " hours attempting to kill " + target.fullName + ".";
        ...
        if(successInfo.success == true) {
            description += "They succeeded in killing them ";

            if(successInfo.quality < 0.25f) {description += "but it was messy and " + target.fullName + " resisted fiercely."; killMultiplier = 0.33f;}
            else if(successInfo.quality < 0.5f) {description += "though it took longer than expected and was not clean."; killMultiplier = 0.66f;}
            else if(successInfo.quality < 0.75f) {description += "with a decisive and controlled strike."; killMultiplier = 1f;}
            else {description += "swiftly and without leaving much trace."; killMultiplier = 1.33f;}
        }
        else {
            description += "Their attempt to kill was not successful ";

            if(successInfo.quality < 0.25f) {description += "they were spotted too early and couldn’t get close."; killMultiplier = 0f;}
            else if(successInfo.quality < 0.5f) {description += target.fullName + " fought them off and forced them to retreat."; killMultiplier = -0.1f;}
            else if(successInfo.quality < 0.75f) {description += "they made too much noise and the attempt failed under pressure."; killMultiplier = -0.2f;}
            else {description += "and they were left injured after the failed attempt."; killMultiplier = -0.3f;}
        }
        ...
        //health changes
        float healthDiff = Mathf.Abs(killMultiplier) * 15f;
        if(killMultiplier < 0)
        {
        ...
        } else if (killMultiplier > 0)
        {
            if(killMultiplier >= 1)
            {
                target.stats.condition = -100f;
                description += "The target was killed by the attacker.";
            }
            else
            {
        ...
        float relChange = 60f * Mathf.Pow(Mathf.Abs(killMultiplier), 1 - Mathf.InverseLerp(0f, 100f, target.attributes.perception));
        ...
        List<Relationship> targetRelationships = dataController.RelationshipPerNPCStorage[target.id];
        for(int i = 0; i < targetRelationships.Count - 1; i++)
        {
            Relationship tRel = targetRelationships[i];
            int otherID;
            ...
            if(tRel.value > 50)
            {
                perfRel.value = Mathf.Clamp(perfRel.value + Mathf.Lerp(-20f, 0f, Mathf.InverseLerp(50f, 100f, tRel.value)), 0f, 100f);
            } else if(tRel.value < 20)
            {
                perfRel.value = Mathf.Clamp(perfRel.value + Mathf.Lerp(0f, 20f, Mathf.InverseLerp(0f, 20f, tRel.value)), 0f, 100f);
            }
        }
        description += " Their relationship was impacted to all that knew who they killed.";

        //happiness changes
        if (performer.traits.Contains(5)) //if they are a serial killer
        {
            performer.stats.happiness = Mathf.Min(100f, performer.stats.happiness + 10f);
        }

        //morality changes
        if(killMultiplier >= 0) performer.attributes.morality = Mathf.Max(0f, performer.attributes.morality - (1f + 9f * killMultiplier)); 
        ...
        if(killMultiplier > 0) {wasPosPerf = true; wasPosRec = false;}
        else {wasPosPerf = false; wasPosRec = false;}

        float severity = 50f * Mathf.Abs(killMultiplier);
