...
[Preserve]
public class ImproveDexterity : SelfAction
{
    private const float attributeGain = 10f;
    public ImproveDexterity() : base('S'){}

    public override char actionType => 'S';
    public override string name => "Improve Dexterity";
    public override string baseDescription => "You can always work on yourself, this IAgent would like to improve their dexterity.";

    protected override float baseTime => 4f;
    protected override float baseEnergy => 20f;
    protected override float complexity => 10f;
    protected override float baseUtility => 34f;
    ...
    protected override float computeUtility(IAgent performer, NoTarget _)
    {
        ...
        effectors.Add(ActionMaths.calcMultiplier(performer.attributes.dexterity, 0f, 100f, 2f, 0.25f)); weights.Add(1f);
    ...
    protected override void innerPerformAction(float percentComplete)
    {
        ...
        if(successInfo.success == true) {
            description += "They successfully honed their dexterity";
            if(successInfo.quality < 0.25f) {description += " though their fingers felt more tangled than nimble."; improvementMultiplier = 0.33f;}
            else if(successInfo.quality < 0.5f) {description += " their movements became a little smoother and more controlled."; improvementMultiplier = 0.66f;}
            else if(successInfo.quality < 0.75f) {description += " their hands moved with practiced precision and confidence."; improvementMultiplier = 1f;}
            else {description += " their movements flowed effortlessly, swift and perfectly coordinated."; improvementMultiplier = 1.33f;}
        }
        else {
            description += "Their dexterity training was not a success";
            if(successInfo.quality < 0.25f) {description += ", they fumbled even the simplest motions."; improvementMultiplier = 0f;}
            else if(successInfo.quality < 0.5f) {description += ", their coordination just wouldn't cooperate."; improvementMultiplier = 0f;}
            else if(successInfo.quality < 0.75f) {description += ", hesitation caused their movements to falter."; improvementMultiplier = -0.05f;}
            else {description += ", their hands seemed determined to work against them."; improvementMultiplier = -0.1f;}
        }

        if(percentComplete != 100f) description += " The dexterity training concluded "+percentComplete.ToString("0.00")+"% of the way through.";
        ...
        if(improvementMultiplier > 0)
        {
            ...
            description += " and they gained "+Mathf.Abs(attributeChange).ToString("0.00")+" dexterity.";
        }
        else
        {
            wasPosPerf = false;
            attributeChange = attributeGain * percentMulti * improvementMultiplier;
            description += " and they lost "+Mathf.Abs(attributeChange).ToString("0.00")+" dexterity.";
        }
        performer.attributes.charisma = Mathf.Clamp(performer.attributes.dexterity + attributeChange, 0f, 100f);
    ...
    protected override float getTimeToComplete(IAgent performer, NoTarget _)
    {
        ...
        effectors.Add(ActionMaths.calcMultiplier(performer.attributes.dexterity, 0f, 100f, 2f, 0.25f)); weights.Add(0.5f);
