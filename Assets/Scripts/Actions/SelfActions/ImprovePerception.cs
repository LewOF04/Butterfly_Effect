...
[Preserve]
public class ImprovePerception : SelfAction
{
    private const float attributeGain = 10f;
    public ImprovePerception() : base('S'){}

    public override char actionType => 'S';
    public override string name => "Improve Perception";
    public override string baseDescription => "You can always work on yourself, this NPC would like to improve their perception.";
    ...
    protected override float computeUtility(NPC performer, NoTarget _)
    {
        ...
        effectors.Add(ActionMaths.calcMultiplier(performer.attributes.perception, 0f, 100f, 2f, 0.25f)); weights.Add(1f);
    ...
    protected override void innerPerformAction(float percentComplete)
    {
        ...
        string description = performer.npcName + " spent " + (percentMulti*timeToComplete).ToString("0.00") + " hours improving their perception.";
        ...
        if(successInfo.success == true) {
            description += "They successfully honed their perception";
            if(successInfo.quality < 0.25f) {description += " but they may have overanalysed things that were never there."; improvementMultiplier = 0.33f;}
            else if(successInfo.quality < 0.5f) {description += " they began noticing small details they once overlooked."; improvementMultiplier = 0.66f;}
            else if(successInfo.quality < 0.75f) {description += " their awareness sharpened, catching subtle shifts in their surroundings."; improvementMultiplier = 1f;}
            else {description += " nothing seems to escape their watchful eye now."; improvementMultiplier = 1.33f;}
        }
        else {
            description += "Their perception training was not a success";
            if(successInfo.quality < 0.25f) {description += ", they misread even the most obvious signs."; improvementMultiplier = 0f;}
            else if(successInfo.quality < 0.5f) {description += ", the details remained frustratingly out of focus."; improvementMultiplier = 0f;}
            else if(successInfo.quality < 0.75f) {description += ", doubt clouded their judgement."; improvementMultiplier = -0.05f;}
            else {description += ", they now question whether they saw things correctly at all."; improvementMultiplier = -0.1f;}
        }

        if(percentComplete != 100f) description += " The perception training concluded "+percentComplete.ToString("0.00")+"% of the way through.";
        ...
        if(improvementMultiplier > 0)
        {
            ...
            description += " and they gained "+Mathf.Abs(attributeChange).ToString("0.00")+" perception.";
        }
        else
        {
            wasPosPerf = false;
            attributeChange = attributeGain * percentMulti * improvementMultiplier;
            description += " and they lost "+Mathf.Abs(attributeChange).ToString("0.00")+" perception.";
        }
        performer.attributes.charisma = Mathf.Clamp(performer.attributes.perception + attributeChange, 0f, 100f);
